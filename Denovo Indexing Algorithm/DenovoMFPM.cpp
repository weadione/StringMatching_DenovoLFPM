...
void DenovoMFPM::FindReadIndex(string frequencyPattern) {	

	indexShortPos.clear();
	deleteShortPos.clear();
	vector<thread> workers;
	string threadShortRead;
	int cutIndex = 0;
	int cutSize= shortReadSet.size() / 8;
	for (int i = 0; true; i++) {	//멀티스레딩을 위한 ShortRead 쪼개기
		if (cutIndex + cutSize + i >= shortReadSet.size()) {
			threadShortRead = shortReadSet.substr(cutIndex);
			workers.push_back(thread(&DenovoMFPM::ThreadFindReadIndex, this, frequencyPattern, threadShortRead, cutIndex, ref(m)));
			break;
		}
		else if (shortReadSet[cutIndex + cutSize + i] == '\n') {
			cutSize = cutSize+ i+1;
			threadShortRead = shortReadSet.substr(cutIndex, cutSize);
			workers.push_back(thread(&DenovoMFPM::ThreadFindReadIndex, this,frequencyPattern, threadShortRead, cutIndex, ref(m)));

			cutIndex = cutIndex + cutSize;
			i = -1;
		}
	}
...
void DenovoMFPM::ThreadFindReadIndex(string frequencyPattern, string shortRead, int rowIndex, mutex& m) {
	//thread::id thisId = this_thread::get_id();
	
	istringstream ss(shortRead);
	string line, window;
	while (ss >> line) {	//\n을 구분자로 ShortRead를 문장단위로 가져옴
		IndexedRead IndexRead(line, rowIndex);
		//cout << thisId << "실행중" << endl;
...
void DenovoMFPM::MergeShortRead() {
...
int DenovoMFPM::QuickPartition(int left, int right) {
	int pivot, intValue;
...
void DenovoMFPM::SortDeletePos(int left, int right) {
	int pivot;
	if (left < right) {
...
void DenovoMFPM::GetAccuracy() {
	...
	cout << "<매칭 결과>" << endl;
	cout << M << " 길이의 DNA를 복원하기 위해" << endl;
	cout << k << " 길이, " << n << "개의 ShortRead를 Denovo 방식으로 매칭한 결과" << endl;
	cout << readNum << "개의 문자열로 매칭됨" << endl;
	while (ss >> line) {	//\n을 구분자로 ShortRead를 문장단위로 가져옴
		num++;
		maxMatch = 0;
		...
		totalAccuracy += ((double)maxMatch / (double)M) * 100;
		cout << num << "번: " << ((double)maxMatch / (double)M) * 100 << "%" << endl;
	}

	//cout << "정확도 합산: 약" << totalAccuracy << "%" << endl;
}
