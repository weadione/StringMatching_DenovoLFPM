...
//DenovoMFPM::FindReadIndex()의 과정에서 찾은 ShortRead들을 객체로 저장하기 위해서 만든 클래스
class IndexedRead {
public:
	...
	bool isFound;			//빈도패턴이 발견 되었는지 유무(DenovoMFPM::FindReadIndex에서 사용됨)
	vector<string> suPrefix;	/*발견된 빈도패턴의 앞뒤로 t만큼의 접두 접미사 부분을 저장
								만약 AGCTTGCAAA에서 TTGC가 패턴이고 t=3이면
								AGC가 접두부 AAA가 접미부로
								결과적으로 AGCAAA가 저장됨*/

	IndexedRead();		//멤버변수 초기화 생성자로 DenovoMFPM::MergeShortRead()의 과정에서 매칭 성공시 사용
	IndexedRead(string value, int rowIndex);	//멤버변수 초기화 생성자로 DenovoMFPM::FindReadIndex()에서 사용
	~IndexedRead();		//소멸자
